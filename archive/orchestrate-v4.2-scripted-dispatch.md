# /orchestrate -- v4.2 scripted dispatch -- public edition (Parked)

From the Skill Bench study (boord-its.com/skills -- every number behind
every rule, including this generation's losses). Status: mechanically proven (100% correct, structurally parallel dispatch, per-worker effort) but 21-48% pricier than v4.1 at equal correctness -- the batch script re-transmits the whole brief set. Parked, not dead.
...
{"mode":"swarm","gate":"GATE line verbatim",
 "workers":[{"id":"W1","nodes":["N01"],"lane":"opus","effort":"high",
             "files":["lib/log.js"],"deps":[],"spec":"SPEC.md sec 2.1-2.4",
             "destructive":false}],
 "batches":[["W1","W2"],["W3"]],
 "gate_cmd":"<the sec 5 script>"}
```

A promoted worker (mundane nodes up-laned for trap density) NAMES its trap:
`"trap":"instance-method logEvent must not change"` -- the promotion is data, not
mood (and the optional hook layer refuses promotions the plan doesn't name).
...
script focused (contracts + named traps + spot-sites + one integration path, <= ~150
lines) -- the per-worker CHECKs already covered the breadth. Do not restate preamble
content in cards; do not narrate between tool calls.
...
- You do NOT implement. Every worker runs in a subagent; you dispatch BATCHES, not
  individuals. "It's quicker to just do this one myself" is the defection the arena
  caught 28/28 times.
...
- **Dispatch each batch as ONE `Workflow` call.** The script is the dispatch -- serial
  spawning is structurally impossible, and each worker gets its lane's model AND
  effort. Template (meta stays a pure literal; scripts are plain JS -- no Date.now /
  Math.random / TypeScript):

```
export const meta = {
  name: 'swarm-batch',
  description: 'Dispatch one planned batch across model/effort lanes',
  phases: [{ title: 'Dispatch' }],
}
const out = await parallel(args.workers.map(w => () =>
  agent(args.preamble + '\n\n' + w.card,
        {label: w.id, model: w.model, effort: w.effort, phase: 'Dispatch'})))
return out.map((r, i) => ({ id: args.workers[i].id, report: r }))
```

  Pass `args = {preamble, workers:[{id, model, effort, card}]}` built straight from
  plan.json + sec 4.5 briefs. The return value carries every worker's report; the
  workflow journal is the ACTUAL-spawn record sec 7 reconciles against.
- **Fallback (no `Workflow` tool in this session):** dispatch the batch as parallel
  `Agent` calls in ONE message -- all in the SAME assistant turn (<= ~10 per message),
  `model` explicit on every spawn, worker id + node ids in `description`. `Agent`
  cannot set worker effort (it inherits) -- a known cost of the fallback, not a reason
  to serialize. Spawning one worker, awaiting its result, then spawning the next is a
  mandate violation, not prudence. There is nothing to check between same-batch
  spawns: landing checks (sec 4.6) run after the WHOLE batch has returned.
...
### 4.4 Lanes (rubric, decided once in the plan -- model AND effort)

| lane | model | effort | goes there |
|---|---|---|---|
| MUNDANE | `haiku` | `low` | mechanical + fully-ruled, WITH the exact rules and trap warnings in the card |
| WORKHORSE | `sonnet` | `medium` | contained implementation; also trap-DENSE mundane clusters |
| CRITICAL | `opus` | `high` | shared unpinned contracts, auth, money, concurrency, destructive |

Worker effort is PART of the lane, not a vibe: a well-carded mundane cluster needs no
deep thinking -- the card already did the thinking (that is the orchestrator's whole
value: the better the brief, the cheaper the worker that clears the gate).
...
mundane cluster dense with anti-mechanical traps runs on `sonnet`, trap named in
plan.json. `subagent_type: general-purpose` always works; a matching domain
specialist (or any domain subagent you have installed) is fine too -- explicit `model` +
`effort` still govern.
...
After the WHOLE batch returns (the Workflow call resolves, or all same-message spawns
return -- never between workers): run `git status --porcelain` + `git diff
--name-only` -- any changed file outside the union of dispatched OWNED lists is
reverted and the owner re-briefed. Spot-check each worker with its own CHECK command
or a 1-line import/run probe. A BLOCKED report gets an answer (from spec/plan), then
a fresh spawn, same lane. A `null` in the workflow's return array is a dead worker --
treat as BLOCKED, re-dispatch. Never accept a green self-report as the gate -- in the
arena a correct diff arrived under a wrong report, and only the script told the truth.
...
Per failing worker -- effort first, model second: (1) fresh spawn, SAME lane at ONE
effort step up (low->medium->high), with the gate failure + current diff of its owned
files pasted in; (2) one lane up (haiku->sonnet->opus) at that lane's default effort,
same evidence; (3) `LADDER_EXHAUSTED` -- you repair the minimal failing diff
in-session and log it in reconciliation. Hard cap: 3 attempts per worker, then stop
and report to the human. Ladder spawns may go through `Agent` directly (a retry is
one worker, not a batch -- effort control is lost there; prefer a 1-worker Workflow
call when effort matters).
...
- Routing table: planned vs ACTUAL spawns (worker / lane / model / effort /
  attempts) -- actuals from the Workflow return/journal, never from memory -- plus any
...
- Only top-level sessions hold spawn tools -- never expect a worker to fan out, never
  delegate orchestration itself. A dispatch script never calls `workflow()` (no
  nesting) and carries no timestamps or randomness.
