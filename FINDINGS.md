...
- **The orchestrator doesn't need max effort — and the ladder kept surprising us.** One notch down from max: every run passed, the planning-heavy fixture got **44%** cheaper, new record at the wall. Two notches down: the wall fixture cracked (one missed site the orchestrator's own gate didn't catch) — so we declared high "the optimum, bracketed both sides." Then we ran the *bottom* rung for completeness, and it went 4-for-4 with the **cheapest wall-regime runs ever recorded** — below the rung that failed. At small k, "bracketed" was a story we told one run too early; the honest state is high-vs-low unresolved, shipped config unchanged until tie-break runs. We also ran the full ladder with the top-tier model in the orchestrator seat: it got cheaper AND faster at every step *down* in effort, finishing an entire swarm in **two orchestrator turns** at minimum effort and taking the all-time wall-clock records — while losing on cost, because its tokens are the priciest. The pattern across worker floor and both ladders is one law: **the more capable the seat, the less deliberate thinking it needs.** Max effort is a tax capable models mostly don't need; correctness risk lives in the *middle* of the ladder, not the bottom.
...
### The plateau (Round 3 closing)

We then built the "best possible" version one more time — the champion text plus a mandatory *counting gate* (every site grep-counted against the inventory, so a missed site can't pass arithmetic — the exact counter to the one gate escape we ever observed). It went 4/4 on correctness and still lost: a dead tie on sweep-shaped work, **+33% on greenfield builds**, because counting is free where sites are grep-countable and pure inflation where they aren't — and the failure it protects against only appears below the effort we ship. That makes four straight challengers the lean skill at high effort has survived (the elaborate suite, scripted dispatch at two efforts, and the counting variant). The honest conclusion: **the text is at a plateau. The remaining headroom isn't prose — it's the unmeasured regimes** (very-large-context tasks, minimum viable brief length, the supervised interactive path). Elaborations that don't target a failure mode your shipped config actually produces are decoration with a price tag.
