...
- **The orchestrator doesn't need max effort — and the ladder has a floor.** The identical skill text with the orchestrator's reasoning effort one notch down passed every run, cut the planning-heavy fixture **44%**, and set a new cheapest-passing record on the 68-site sweep. Maximum thinking depth was paying for decisions the skill had already made deterministic. We then ran the next rung down: at *medium*, the scale-wall fixture cracked (one missed site of 68 — which the orchestrator's own gate script failed to catch: judgment-thinning shows up first as a shallower gate) and the planning-heavy fixture cost *more* than at high, because an under-thinking orchestrator types more, not less. The effort curve is U-shaped; correctness pays the downside first. **High is the optimum, bracketed from both sides.**
