...
- [`examples/eval/`](../examples/eval/) — **a real 3-arm experiment** (monolith vs. naive-parallel vs. dependency-ordered), mypy-scored on a live codebase. It confirms this document's central hedge empirically: on a small task the **monolith was cheapest *and* correct**, while dependency-ordered state-forwarding was the difference between 0/3 and 3/3 *once work was parallelized*. Coordination earns its cost only above the size where one window stops being enough.
